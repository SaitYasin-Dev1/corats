...
export function PluginPickerTrigger({
  ...
  return (
    <button
      ...
      disabled={disabled}
      className={cn(
        "flex flex-row items-center gap-2 rounded-full px-2.5 py-1 text-content",
        formControlBorderClassName,
        formControlSurfaceClassName,
        formControlTransitionClassName,
        disabled
    ...
    >
      ...
      {count > 0 ? (
        <span
          ...
          className="inline-flex h-4 min-w-4 items-center justify-center rounded-full bg-[#1F1D17] px-1 text-[11px] font-semibold leading-none text-white"
        >
          {count}
        </span>
      ) : null}
